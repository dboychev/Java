...
public class Simulation1
{
	...
	public static void main(String[] args)
	{
        ...
        for (int minute = 1; minute <= SIMULATION_TIME; minute++)
        {
        	...
        	for (int i = 0; i < 3; i++)
        	{
        		if (!q.isEmpty() && registerAvailableTimes[i] <= minute)
	        	{
	        		Customer ready = q.remove();
	        		numCustomers++;
	        		totalWaitTime += minute - ready.getTimeEnteredQueue();
	        		registerAvailableTimes[i] = minute + ready.getTimeNeededAtRegister();
	        	}
        	}
        }
